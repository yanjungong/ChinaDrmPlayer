...
int32_t ChinaDrmParser::setLicense(const std::string& drmLicense, const std::string uid, int32_t instanceId){
    UNILOGD("setLicense(drmLicense=%s, uid=%s, instanceId=%d)", drmLicense.c_str(), uid.c_str(), instanceId);
    ...
    int32_t ret = monalisa_set_license((char*)(drmLicense.c_str()), int32_t(drmLicense.length()), instanceId,
                                   (unsigned char* )(uid.c_str()));
